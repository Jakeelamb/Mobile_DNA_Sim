...
pub fn render_sim_widgets(tab_state: &TabState) -> Vec<AppWidget> {
   ...
   // Convert the Vec<Span> into Vec<Line> (or directly into Text)
    ...
    let species_info_text = vec![
        Line::from(vec![Span::raw(format!("Species: {}\n", tab_state.current_species))]),
        Line::from(vec![Span::raw(format!("Start Genome Size: {}\n", tab_state.start_genome_size))]),
        Line::from(vec![Span::raw(format!("Start Exon Size: {}\n", tab_state.start_exon_size))]),
        Line::from(vec![Span::raw(format!("Exon/Genome Ratio: {}\n", tab_state.exon_genome_ratio))]),
        Line::from(vec![Span::raw(format!("# of Simulation Rounds completed: {}\n", tab_state.simulation_rounds_completed))]),
        Line::from(vec![Span::raw(format!("# of TEs mobilized: {}\n", tab_state.tes_mobilized))]),
        Line::from(vec![Span::raw(format!("# of Mutations: {}\n", tab_state.mutations))]),
        Line::from(vec![Span::raw(format!("Current Genome Size: {}\n", tab_state.current_genome_size))]),
        Line::from(vec![Span::raw(format!("Current Exon/Genome Ratio: {}\n", tab_state.current_exon_genome_ratio))]),
        Line::from(vec![Span::raw(format!("Current Probability of TE causing Mutation: {}\n", tab_state.probability_of_te_mutation))]),
    ];
