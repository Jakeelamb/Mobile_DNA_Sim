...
#set up logging
# Change the logging level in the basicConfig to reduce the verbosity
logging.basicConfig(filename='simulation.log.txt', filemode='w', level=logging.WARNING, format='%(asctime)s - %(levelname)s - %(message)s')
#set up directories
...
def populate_grid(grid, elements):
    for element_type, lengths in elements.items():
        for length in lengths:
            length = int(length)  # Ensure the length is an integer
            placed = False

            # Attempt to place the element on the grid
            while not placed:
                # Randomly choose a strand (row in the grid)
                strand = np.random.randint(0, grid.shape[0])  # 0 or 1 for a 2-row grid

                # Randomly choose a starting position, ensuring there's enough space
                start_pos = np.random.randint(0, grid.shape[1] - length)

                # Check if the selected position is free
                if grid[strand, start_pos:start_pos + length].sum() == 0:
                    # Place the element by setting the corresponding grid positions to the element type
                    grid[strand, start_pos:start_pos + length] = element_type
                    placed = True
                # If the position is not free, the loop continues for another attempt

            # Optional logging to track placement
            logging.debug(f"Placed element of type {element_type} of length {length} at position ({strand}, {start_pos})")
...
def move_element(grid, element_type, interaction_log, round_num, element_length):
    num_moved_elements = 0
    num_interactions = 0
    ...
    for pos in element_positions:
        ...
        # Determine new position for the element
        new_strand = np.random.randint(0, grid.shape[0])
        new_start_pos = np.random.randint(0, grid.shape[1] - element_length + 1)

        # Check and log interactions for the entire range of the new position
        for i in range(new_start_pos, new_start_pos + element_length):
            if 0 <= i < grid.shape[1]:
                adjacent_element = grid[new_strand, i]
                if adjacent_element != 0:  # Check if the space is not empty
                    interaction_type = (element_type, adjacent_element)
                    interaction_log[round_num][interaction_type] += 1
                    num_interactions += 1

        # Insert the element at the new position range
        grid[new_strand, new_start_pos:new_start_pos + element_length] = element_type

        # Remove the element from its original position range
...
def run_simulation(grid, num_rounds, exon_lengths, retrotransposons_lengths, dnatransposons_lengths, non_coding_segment_lengths):
    ...
    for round_num in range(num_rounds):
        ...
        # Create a list of element types to move and shuffle it
        element_types_to_move = [2, 3]
        np.random.shuffle(element_types_to_move)

        # Simulate the movements in the shuffled order and record interactions
        for element_type in element_types_to_move:
            logging.info(f"Attempting to move elements of type {element_type}")
            move_element(grid, element_type, interaction_log, round_num)
            logging.info(f"Finished attempting to move elements of type {element_type}")

        # Plotting and saving the figure for each round
        plt.figure(figsize=(10, 6))
        ...
        plt.title(f'Interaction Trends - Round {round_num}')
        plt.xticks(np.arange(len(interaction_counts)) + width / 2, range(num_rounds))
        plt.legend()
        plt.tight_layout()
        plt.savefig(f'Figures/interaction_trends_round_{round_num}_bar_chart.png')
        plt.close()
...
num_of_threads = 10  # Number of threads to use
num_rounds = 1       # Number of rounds per simulation

# Using ThreadPoolExecutor to run simulations in parallel
with ThreadPoolExecutor(max_workers=num_of_threads) as executor:
    # Create a list of futures
    futures = [executor.submit(single_simulation_run, i) for i in range(num_of_threads)]
